...
private void miDestroyPictureClip(PicturePtr pPicture)
{
...
private int miChangePictureClip(PicturePtr pPicture, int type, void* value, int n)
{
    ...
    case CT_PIXMAP:
        ...
        clientClip = BitmapToRegion(pScreen, cast(PixmapPtr) value);
        if (!clientClip)
            return BadAlloc;
...
private void miChangePicture(PicturePtr pPicture, Mask mask)
{
    return;
}

private void miValidatePicture(PicturePtr pPicture, Mask mask)
{
...
private int miChangePictureTransform(PicturePtr pPicture, PictTransform* transform)
{
    return Success;
}
...
enum string BOUND(string v) = `cast(INT16) ((` ~ v ~ `) < MINSHORT ? MINSHORT : (` ~ v ~ `) > MAXSHORT ? MAXSHORT : (` ~ v ~ `))`;

pragma(inline, true) private pixman_bool_t miClipPictureReg(pixman_region16_t* pRegion, pixman_region16_t* pClip, int dx, int dy)
{
    if (assumeNoGC(&pixman_region_n_rects)(pRegion) == 1 &&
        assumeNoGC(&pixman_region_n_rects)(pClip) == 1) {
        pixman_box16_t* pRbox = assumeNoGC(&pixman_region_rectangles)(pRegion, null);
        pixman_box16_t* pCbox = assumeNoGC(&pixman_region_rectangles)(pClip, null);
        int v = void;

        if (pRbox.x1 < (v = pCbox.x1 + dx))
            pRbox.x1 = mixin(BOUND!(`v`));
        if (pRbox.x2 > (v = pCbox.x2 + dx))
            pRbox.x2 = mixin(BOUND!(`v`));
        if (pRbox.y1 < (v = pCbox.y1 + dy))
            pRbox.y1 = mixin(BOUND!(`v`));
        if (pRbox.y2 > (v = pCbox.y2 + dy))
            pRbox.y2 = mixin(BOUND!(`v`));
        if (pRbox.x1 >= pRbox.x2 || pRbox.y1 >= pRbox.y2) {
            assumeNoGC(&pixman_region_init)(pRegion);
        }
    }
    else if (!assumeNoGC(&pixman_region_not_empty)(pClip))
        return FALSE;
    else {
        if (dx || dy)
            assumeNoGC(&pixman_region_translate)(pRegion, -dx, -dy);
        if (!assumeNoGC(&pixman_region_intersect)(pRegion, pRegion, pClip))
            return FALSE;
        if (dx || dy)
            assumeNoGC(&pixman_region_translate)(pRegion, dx, dy);
    }
    return assumeNoGC(&pixman_region_not_empty)(pRegion);
}

pragma(inline, true) private Bool miClipPictureSrc(RegionPtr pRegion, PicturePtr pPicture, int dx, int dy)
{
    if (pPicture.clientClip) {
        Bool result = void;

        assumeNoGC(&pixman_region_translate)(pPicture.clientClip,
...
private void SourceValidateOnePicture(PicturePtr pPicture)
{
    DrawablePtr pDrawable = pPicture.pDrawable;
...
Bool miComputeCompositeRegion(RegionPtr pRegion, PicturePtr pSrc, PicturePtr pMask, PicturePtr pDst, INT16 xSrc, INT16 ySrc, INT16 xMask, INT16 yMask, INT16 xDst, INT16 yDst, CARD16 width, CARD16 height)
{
    ...
    v = xDst + width;
    pRegion.extents.x2 = mixin(BOUND!(`v`));
    ...
    if (!miClipPictureReg(pRegion, pDst.pCompositeClip, 0, 0)) {
        assumeNoGC(&pixman_region_fini)(pRegion);
        return FALSE;
    }
    if (pDst.alphaMap) {
...
void miRenderColorToPixel(PictFormatPtr format, xRenderColor* color, CARD32* pixel)
{
    ...
    case PictTypeIndexed:
        pIndexed = cast(miIndexedPtr) (format.index.devPrivate);
        if (pIndexed.color) {
            r = color.red >> 11;
            g = color.green >> 11;
            b = color.blue >> 11;
...
private CARD16 miFillColor(CARD32 pixel, int bits)
{
    while (bits < 16) {
        pixel |= pixel << bits;
        bits <<= 1;
    }
    return cast(CARD16) pixel;
}

Bool miIsSolidAlpha(PicturePtr pSrc)
{
    ScreenPtr pScreen = void;
    char[1] line = void;
    ...
    if (pSrc.pDrawable.width != 1 || pSrc.pDrawable.height != 1)
        return FALSE;
    line[0] = 1;
    (*pScreen.GetImage) (pSrc.pDrawable, 0, 0, 1, 1, ZPixmap, ~0L, line.ptr);
    switch (pSrc.pDrawable.bitsPerPixel) {
    case 1:
        return cast(CARD8) line[0] == 1 || cast(CARD8) line[0] == 0x80;
    case 4:
        return cast(CARD8) line[0] == 0xf || cast(CARD8) line[0] == 0xf0;
    case 8:
        return cast(CARD8) line[0] == 0xff;
    default:
        return FALSE;
    }
}
...
Bool miPictureInit(ScreenPtr pScreen, PictFormatPtr formats, int nformats)
{
