...
uint XkbIndicatorsToUpdate(DeviceIntPtr dev, c_ulong state_changes, Bool enable_changes)
{
    uint update = 0;
...
private Bool XkbApplyLEDChangeToKeyboard(XkbSrvInfoPtr xkbi, XkbIndicatorMapPtr map, Bool on, XkbChangesPtr change)
{
    ...
    if ((map.groups) && ((map.which_groups & (~XkbIM_UseBase)) != 0)) {
        ...
        if (map.which_groups & (XkbIM_UseLocked | XkbIM_UseEffective)) {
            ...
            state.locked_group = cast(ubyte)i;
            stateChange = TRUE;
        }
    ...
    if ((map.mods.mask) && ((map.which_mods & (~XkbIM_UseBase)) != 0)) {
        ...
        if (map.which_mods & (XkbIM_UseLatched | XkbIM_UseEffective)) {
            ...
            if (newmods != state.locked_mods) {
                newmods &= map.mods.mask;
                XkbLatchModifiers(xkbi.device, map.mods.mask, cast(ubyte)newmods);
                stateChange = TRUE;
            }
        }
    }
...
private void XkbUpdateLedAutoState(DeviceIntPtr dev, XkbSrvLedInfoPtr sli, uint maps_to_check, xkbExtensionDeviceNotify* ed, XkbChangesPtr changes, XkbEventCausePtr cause)
{
    ...
    if (ed is null) {
        ...
        memset(cast(char*) ed, 0, xkbExtensionDeviceNotify.sizeof);
             ...
             ((ed.ledClass != sli.class_) || (ed.ledID != sli.id))) {
    ...
    ed.ledClass = sli.class_;
    ed.ledID = sli.id;
    ed.ledsDefined = sli.namesPresent | sli.mapsPresent;
    ed.ledState = sli.effectiveState;
    ed.unsupported = 0;
    ed.supported = cast(ushort)cast(ushort)XkbXI_AllFeaturesMask;
...
void XkbSetIndicators(DeviceIntPtr dev, CARD32 affect, CARD32 values, XkbEventCausePtr cause)
{
    XkbSrvLedInfoPtr sli = void;
    XkbChangesRec changes = { 0 };
    xkbExtensionDeviceNotify ed = { 0 };
    uint side_affected = void;
...
void XkbUpdateIndicators(DeviceIntPtr dev, CARD32 update, Bool check_edevs, XkbChangesPtr changes, XkbEventCausePtr cause)
{
    XkbSrvLedInfoPtr sli = void;
...
void XkbForceUpdateDeviceLEDs(DeviceIntPtr dev)
{
...
XkbSrvLedInfoPtr XkbAllocSrvLedInfo(DeviceIntPtr dev, KbdFeedbackPtr kf, LedFeedbackPtr lf, uint needed_parts)
{
    ...
    if ((kf != null) && (kf.xkb_sli is null)) {
        ...
        if (dev.key && dev.key.xkbInfo)
            ...
            sli.flags = 0;
        sli.class_ = KbdFeedbackClass;
        sli.id = kf.ctrl.id;
        sli.fb.kf = kf;

        sli.autoState = 0;
        sli.explicitState = cast(uint)kf.ctrl.leds;
        sli.effectiveState = cast(uint)kf.ctrl.leds;
        ...
        if ((kf == dev.kbdfeed) && (dev.key) && (dev.key.xkbInfo)) {
            ...
            sli.physIndicators = cast(uint)xkb.indicators.phys_indicators;
            sli.names = xkb.names.indicators.ptr;
        ...
        sli = kf.xkb_sli;
        sli.physIndicators = cast(uint)xkb.indicators.phys_indicators;
        if (xkb.names.indicators.ptr !is sli.names) {
            checkNames = TRUE;
            sli.names = xkb.names.indicators.ptr;
        }
        if (xkb.indicators.maps.ptr !is sli.maps) {
            checkAccel = TRUE;
            sli.maps = xkb.indicators.maps.ptr;
        }
    }
        ...
        if (dev.key && dev.key.xkbInfo)
            ...
            sli.flags = 0;
        sli.class_ = LedFeedbackClass;
        sli.id = lf.ctrl.id;
        sli.fb.lf = lf;

        sli.physIndicators = cast(uint)lf.ctrl.led_mask;
        sli.autoState = 0;
        sli.explicitState = cast(uint)lf.ctrl.led_values;
        sli.effectiveState = cast(uint)lf.ctrl.led_values;
        sli.maps = null;
        sli.names = null;
    }
    else
 ...
 * XkbFreeSrvLedInfo.
 */
XkbSrvLedInfoPtr XkbCopySrvLedInfo(DeviceIntPtr from, XkbSrvLedInfoPtr src, KbdFeedbackPtr kf, LedFeedbackPtr lf)
{
    ...
    memcpy(sli_new, src, XkbSrvLedInfoRec.sizeof);
    if (sli_new.class_ == KbdFeedbackClass)
        sli_new.fb.kf = kf;
    else
        sli_new.fb.lf = lf;
...
void XkbFlushLedEvents(DeviceIntPtr dev, DeviceIntPtr kbd, XkbSrvLedInfoPtr sli, xkbExtensionDeviceNotify* ed, XkbChangesPtr changes, XkbEventCausePtr cause)
{
    if (changes) {
        ...
        if (mixin(XkbAX_NeedFeedback
            !("kbd.key.xkbInfo.desc.ctrls", "XkbAX_IndicatorFBMask"))) {
            if (sli.effectiveState)
                /* it appears that the which parameter is not used */
                XkbDDXAccessXBeep(dev, _BEEP_LED_ON, cast(uint)XkbAccessXFeedbackMask);
            else
                XkbDDXAccessXBeep(dev, _BEEP_LED_OFF, cast(uint)XkbAccessXFeedbackMask);
        }
    }
    if (ed) {
        if (ed.reason) {
...
void XkbApplyLedNameChanges(DeviceIntPtr dev, XkbSrvLedInfoPtr sli, uint changed_names, xkbExtensionDeviceNotify* ed, XkbChangesPtr changes, XkbEventCausePtr cause)
{
    ...
    ed.ledState = sli.effectiveState;
    ed.unsupported = 0;
    ed.supported = cast(ushort)XkbXI_AllFeaturesMask;
