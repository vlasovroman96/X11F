...
Bool glamor_set_destination_drawable(DrawablePtr drawable, int box_index, Bool do_drawable_translate, Bool center_offset, GLint matrix_uniform_location, int* p_off_x, int* p_off_y)
{
    ...
    int w = box.x2 - box.x1;
    int h = box.y2 - box.y1;
    float scale_x = 2.0f / cast(float) w;
    float scale_y = 2.0f / cast(float) h;
    float center_adjust = 0.0f;
    glamor_pixmap_fbo* pixmap_fbo = void;
...
void glamor_set_color_depth(ScreenPtr pScreen, int depth, CARD32 pixel, GLint uniform)
{
...
Bool glamor_set_texture_pixmap(PixmapPtr texture, Bool destination_red)
{
    glamor_pixmap_private* texture_priv = void;
...
Bool glamor_set_texture(PixmapPtr texture, Bool destination_red, int off_x, int off_y, GLint offset_uniform, GLint size_inv_uniform)
{
...
Bool glamor_set_tiled(DrawablePtr drawable, GCPtr gc, GLint offset_uniform, GLint size_inv_uniform)
{
...
Bool glamor_set_stippled(DrawablePtr drawable, GCPtr gc, GLint fg_uniform, GLint offset_uniform, GLint size_uniform)
{
    PixmapPtr stipple = void;
