 ...
 */
void glamor_upload_boxes(DrawablePtr drawable, BoxPtr in_boxes, int in_nbox, int dx_src, int dy_src, int dx_dst, int dy_dst, ubyte* bits, uint byte_stride)
{
    ...
    int bytes_per_pixel = cast(int)PIXMAN_FORMAT_BPP(f.render_format) >> 3;
    char* tmp_bits = null;
    ...
    mixin(glamor_pixmap_loop!("priv", "box_index", q{
        ...
        while (nbox--) {
            ...
            src_line = cast(uint*)(bits + ofs);

            if (tmp_bits) {
                uint* tmp_line = cast(uint*)(tmp_bits + ofs);
                int x = void, y = void;
                ...
                src_line = cast(uint*)(tmp_bits + ofs);
            }
...
void glamor_upload_region(DrawablePtr drawable, RegionPtr region, int region_x, int region_y, ubyte* bits, uint byte_stride)
{
 ...
 */
void glamor_download_boxes(DrawablePtr drawable, BoxPtr in_boxes, int in_nbox, int dx_src, int dy_src, int dx_dst, int dy_dst, ubyte* bits, uint byte_stride)
{
    ScreenPtr screen = drawable.pScreen;
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);
    PixmapPtr pixmap = glamor_get_drawable_pixmap(drawable);
