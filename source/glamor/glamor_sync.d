...
private DevPrivateKeyRec glamor_sync_fence_key;

struct glamor_sync_fence {
        SyncFenceSetTriggeredFunc set_triggered;
};

pragma(inline, true) private glamor_sync_fence* glamor_get_sync_fence(SyncFence* fence)
{
    return cast(glamor_sync_fence*) dixLookupPrivate(&fence.devPrivates, &glamor_sync_fence_key);
}

private void glamor_sync_fence_set_triggered(SyncFence* fence)
{
	ScreenPtr screen = fence.pScreen;
	glamor_screen_private* glamor = glamor_get_screen_private(screen);
	glamor_sync_fence* glamor_fence = glamor_get_sync_fence(fence);
