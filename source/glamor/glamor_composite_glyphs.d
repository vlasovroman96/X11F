module glamor.glamor_composite_glyphs;
@nogc nothrow:
extern(C): __gshared:
/*
...
private DevPrivateKeyRec glamor_glyph_private_key;
...
private Bool glamor_glyph_can_add(glamor_glyph_atlas* atlas, int dim, DrawablePtr glyph_draw)
{
    /* Step down */
...
private Bool glamor_glyph_add(glamor_glyph_atlas* atlas, DrawablePtr glyph_draw)
{
...
private const(glamor_facet) glamor_facet_composite_glyphs_120 = {
    name: "composite_glyphs",
...
private Bool glamor_glyphs_init_facet(ScreenPtr screen)
{
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);
...
private void glamor_glyphs_fini_facet(ScreenPtr screen)
{
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);
...
private void glamor_glyphs_flush(CARD8 op, PicturePtr src, PicturePtr dst, glamor_program* prog, glamor_glyph_atlas* atlas, int nglyph)
{
...
private GLshort* glamor_glyph_start(ScreenPtr screen, int count)
{
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);
    GLshort* v = void;
    ...
    if (glamor_glsl_has_ints(glamor_priv)) {
        ...
        glEnableVertexAttribArray(GLAMOR_VERTEX_SOURCE);
        glVertexAttribPointer(GLAMOR_VERTEX_SOURCE, 2, GL_SHORT, GL_FALSE,
                              4 * GLshort.sizeof, vbo_offset + 2 * GLshort.sizeof);
    }
    return v;
}

pragma(inline, true) private glamor_glyph_atlas* glamor_atlas_for_glyph(glamor_screen_private* glamor_priv, DrawablePtr drawable)
{
...
void glamor_composite_glyphs(CARD8 op, PicturePtr src, PicturePtr dst, PictFormatPtr glyph_format, INT16 x_src, INT16 y_src, int nlist, GlyphListPtr list, GlyphPtr* glyphs)
{
    int glyphs_queued = void;
    GLshort* v = null;
    DrawablePtr drawable = dst.pDrawable;
    ...
    glamor_glyph_atlas* glyph_atlas = null;
    int x = 0, y = 0;
    int n = void;
        ...
        while (n--) {
            ...
            if (glyph.info.width && glyph.info.height) {
                ...
                if (_X_UNLIKELY(glyph_draw.width > glyph_max_dim ||
                                ...
                                !glamor_pixmap_is_memory(cast(PixmapPtr)glyph_draw)))
                {
                    if (glyphs_queued) {
                ...
                bail_one:
                    ...
                    glamor_glyph_atlas* next_atlas = glamor_atlas_for_glyph(glamor_priv, glyph_draw);
...
private glamor_glyph_atlas* glamor_alloc_glyph_atlas(ScreenPtr screen, int depth, CARD32 f)
{
    PictFormatPtr format = void;
    glamor_glyph_atlas* glyph_atlas = void;

    format = PictureMatchFormat(screen, depth, f);
    if (!format)
        return null;
    glyph_atlas = cast(glamor_glyph_atlas*) calloc (1, glamor_glyph_atlas.sizeof);
    if (!glyph_atlas)
        return null;
...
Bool glamor_composite_glyphs_init(ScreenPtr screen)
{
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);

    if (!dixRegisterPrivateKey(&glamor_glyph_private_key, PRIVATE_PIXMAP, glamor_glyph_private.sizeof))
        return FALSE;
...
private void glamor_free_glyph_atlas(glamor_glyph_atlas* atlas)
{
    if (!atlas)
        return;
...
void glamor_composite_glyphs_fini(ScreenPtr screen)
{
    glamor_screen_private* glamor_priv = glamor_get_screen_private(screen);
