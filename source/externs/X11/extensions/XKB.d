module externs.X11.extensions.XKB;
@nogc nothrow:
extern(C): __gshared:
/************************************************************
...
enum string	XkbLegalXILedClass(string c) = `(((` ~ c ~ `)==KbdFeedbackClass)||
					((` ~ c ~ `)==LedFeedbackClass)||
					((` ~ c ~ `)==XkbDfltXIClass)||
					((` ~ c ~ `)==XkbAllXIClasses))`;
enum string	XkbLegalXIBellClass(string c) = `(((` ~ c ~ `)==KbdFeedbackClass)||
					((` ~ c ~ `)==BellFeedbackClass)||
					((` ~ c ~ `)==XkbDfltXIClass)||
					((` ~ c ~ `)==XkbAllXIClasses))`;
enum string	XkbExplicitXIDevice(string c) = `(((` ~ c ~ `)&(~0xff))==0)`;
enum string	XkbExplicitXIClass(string c) = `(((` ~ c ~ `)&(~0xff))==0)`;
enum string	XkbExplicitXIId(string c) = `(((` ~ c ~ `)&(~0xff))==0)`;
enum string	XkbSingleXIClass(string c) = `((((` ~ c ~ `)&(~0xff))==0)||((` ~ c ~ `)==XkbDfltXIClass))`;
enum string	XkbSingleXIId(string c) = `((((` ~ c ~ `)&(~0xff))==0)||((` ~ c ~ `)==XkbDfltXIId))`;
...
enum string XkbGroupForCoreState(string s) = `(((` ~ s ~ `)>>13)&0x3)`;
enum string	XkbIsLegalGroup(string g) = `(((` ~ g ~ `)>=0)&&((` ~ g ~ `)<XkbNumKbdGroups))`;
...
enum string	XkbSA_ValOp(string a) = `((` ~ a ~ `)&XkbSA_ValOpMask)`;
enum string	XkbSA_ValScale(string a) = `((` ~ a ~ `)&XkbSA_ValScaleMask)`;
...
enum	XkbSA_BreakLatch = 
     ...
     */
enum string	XkbIsModAction(string a) = `(((` ~ a ~ `).type>=Xkb_SASetMods)&&((` ~ a ~ `).type<=XkbSA_LockMods))`;
enum string	XkbIsGroupAction(string a) = `(((` ~ a ~ `).type>=XkbSA_SetGroup)&&((` ~ a ~ `).type<=XkbSA_LockGroup))`;
...
enum string	XkbIsLegalKeycode(string k) = `((` ~ k ~ `)>=XkbMinLegalKeyCode)`;
...
enum string	XkbShiftLevel(string n) = `((` ~ n ~ `)-1)`;
enum string	XkbShiftLevelMask(string n) = `(1<<((` ~ n ~ `)-1))`;
