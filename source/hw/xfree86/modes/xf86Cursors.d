 ...
 */
private Rotation xf86_crtc_cursor_rotation(xf86CrtcPtr crtc)
{
 ...
 */
private void xf86_crtc_rotate_coord(Rotation rotation, int width, int height, int x_dst, int y_dst, int* x_src, int* y_src)
{
    int t = void;
 ...
 */
private void xf86_crtc_rotate_coord_back(Rotation rotation, int width, int height, int x_dst, int y_dst, int* x_src, int* y_src)
{
    int t = void;
 ...
 */
private cursor_bit cursor_bitpos(CARD8* image, xf86CursorInfoPtr cursor_info, int x, int y, Bool mask)
{
    const(int) flags = cursor_info.Flags;
    const(Bool) interleaved = ! !(flags & (HARDWARE_CURSOR_SOURCE_MASK_INTERLEAVE_1 |
 ...
 */
private CARD8 get_bit(CARD8* image, xf86CursorInfoPtr cursor_info, int x, int y, Bool mask)
{
    cursor_bit bit = cursor_bitpos(image, cursor_info, x, y, mask);

    return (*bit.byte_ >> bit.bitpos) & 1;
}

/*
 * Set one bit in a cursor bitmap
 */
private void set_bit(CARD8* image, xf86CursorInfoPtr cursor_info, int x, int y, Bool mask)
{
    cursor_bit bit = cursor_bitpos(image, cursor_info, x, y, mask);

    *bit.byte_ |= 1 << bit.bitpos;
}
 ...
 * *_cursor_*_check
 */
pragma(inline, true) private Bool xf86_driver_has_load_cursor_image(xf86CrtcPtr crtc)
{
    return crtc.funcs.load_cursor_image_check || crtc.funcs.load_cursor_image;
}

pragma(inline, true) private Bool xf86_driver_has_load_cursor_argb(xf86CrtcPtr crtc)
{
    return crtc.funcs.load_cursor_argb_check || crtc.funcs.load_cursor_argb;
}

pragma(inline, true) private Bool xf86_driver_show_cursor(xf86CrtcPtr crtc)
{
...
pragma(inline, true) private Bool xf86_driver_load_cursor_image(xf86CrtcPtr crtc, CARD8* cursor_image)
{
...
pragma(inline, true) private Bool xf86_driver_load_cursor_argb(xf86CrtcPtr crtc, CARD32* cursor_argb)
{
...
private void xf86_set_cursor_colors(ScrnInfoPtr scrn, int bg, int fg)
{
    ...
    xf86_config.cursor_fg = cast(CARD32) fg | 0xff000000;
    xf86_config.cursor_bg = cast(CARD32) bg | 0xff000000;
...
Bool xf86_crtc_show_cursor(xf86CrtcPtr crtc)
{
...
private void xf86_crtc_transform_cursor_position(xf86CrtcPtr crtc, int* x, int* y)
{
    ScrnInfoPtr scrn = crtc.scrn;
    ScreenPtr screen = scrn.pScreen;
    xf86CrtcConfigPtr xf86_config = XF86_CRTC_CONFIG_PTR(scrn);
