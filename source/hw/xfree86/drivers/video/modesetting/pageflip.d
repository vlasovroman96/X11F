 ...
 */
private void ms_pageflip_free(ms_crtc_pageflip* flip)
{
...
alias QUEUE_FLIP_SUCCESS = queue_flip_status.QUEUE_FLIP_SUCCESS;
alias QUEUE_FLIP_ALLOC_FAILED = queue_flip_status.QUEUE_FLIP_ALLOC_FAILED;
alias QUEUE_FLIP_QUEUE_ALLOC_FAILED = queue_flip_status.QUEUE_FLIP_QUEUE_ALLOC_FAILED;
alias QUEUE_FLIP_DRM_FLUSH_FAILED = queue_flip_status.QUEUE_FLIP_DRM_FLUSH_FAILED;
...
enum MS_ASYNC_FLIP_LOG_ENABLE_LOGS_INTERVAL_MS = 10000;
enum MS_ASYNC_FLIP_LOG_FREQUENT_LOGS_INTERVAL_MS = 1000;
enum MS_ASYNC_FLIP_FREQUENT_LOG_COUNT = 10;

private void ms_print_pageflip_error(int screen_index, const(char)* log_prefix, int crtc_index, int flags, int err)
{
...
private Bool ms_tearfree_dri_flip(modesettingPtr ms, xf86CrtcPtr crtc, void* event, ms_pageflip_handler_proc pageflip_handler, ms_pageflip_abort_proc pageflip_abort)
{
    ...
    flip = cast(ms_crtc_pageflip*) calloc(1, typeof(*flip).sizeof);
    if (!flip)
        return FALSE;

    flipdata = cast(ms_flipdata*) calloc(1, typeof(*flipdata).sizeof);
    if (!flipdata) {
        free(flip);
        return FALSE;
    }
...
Bool ms_do_pageflip(ScreenPtr screen, PixmapPtr new_front, void* event, xf86CrtcPtr ref_crtc, Bool async, ms_pageflip_handler_proc pageflip_handler, ms_pageflip_abort_proc pageflip_abort, const(char)* log_prefix)
{
    ...
    for (i = 0; i < config.num_crtc; i++) {
        queue_flip_status flip_status = void;
...
private void ms_tearfree_dri_notify(drmmode_tearfree_ptr trf, ulong msc, ulong usec)
{
    ms_crtc_pageflip* flip = void, tmp = void;
