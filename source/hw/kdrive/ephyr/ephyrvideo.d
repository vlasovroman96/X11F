...
alias EphyrXVPriv = _EphyrXVPriv;
...
alias EphyrPortPriv = _EphyrPortPriv;



private EphyrXVPriv* ephyrXVPrivNew();
...
private int s_base_port_id;

/**************
 * <helpers>
 * ************/

private Bool adaptor_has_flags(const(xcb_xv_adaptor_info_t)* adaptor, uint flags)
{
    return (adaptor.type & flags) == flags;
}

private Bool ephyrLocalAtomToHost(int a_local_atom, int* a_host_atom)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_intern_atom_cookie_t cookie = void;
...
private Bool translate_video_encodings(KdVideoAdaptorPtr adaptor, xcb_xv_adaptor_info_t* host_adaptor)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    int i = void;
    xcb_xv_query_encodings_cookie_t cookie = void;
    xcb_xv_query_encodings_reply_t* reply = void;
    xcb_xv_encoding_info_iterator_t encoding_it = void;
...
private Bool translate_xv_attributes(KdVideoAdaptorPtr adaptor, xcb_xv_adaptor_info_t* host_adaptor)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    int i = 0;
    xcb_xv_attribute_info_iterator_t it = void;
    xcb_xv_query_port_attributes_cookie_t cookie = xcb_xv_query_port_attributes(conn, host_adaptor.base_id);
    xcb_xv_query_port_attributes_reply_t* reply = xcb_xv_query_port_attributes_reply(conn, cookie, null);
...
private Bool translate_xv_image_formats(KdVideoAdaptorPtr adaptor, xcb_xv_adaptor_info_t* host_adaptor)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    int i = 0;
    xcb_xv_list_image_formats_cookie_t cookie = xcb_xv_list_image_formats(conn, host_adaptor.base_id);
    xcb_xv_list_image_formats_reply_t* reply = xcb_xv_list_image_formats_reply(conn, cookie, null);
    xcb_xv_image_format_info_t* formats = void;
...
private Bool ephyrXVPrivQueryHostAdaptors(EphyrXVPriv* a_this)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    ...
    for (i = 0; i < a_this.num_adaptors; i++) {
        ...
        for (j = 0; j < cur_host_adaptor.num_formats; j++) {
            ...
            a_this.adaptors[i].pFormats[j].class_ = visual._class;
        }
...
private Bool ephyrXVPrivSetAdaptorsHooks(EphyrXVPriv* a_this)
{
    int i = 0;
    xcb_xv_adaptor_info_iterator_t it = void;
...
private Bool ephyrXVPrivRegisterAdaptors(EphyrXVPriv* a_this, ScreenPtr a_screen)
{
    Bool is_ok = FALSE;
...
private Bool ephyrXVPrivIsAttrValueValid(XvAttributePtr a_attrs, int a_attrs_len, const(char)* a_attr_name, int a_attr_value, Bool* a_is_valid)
{
    int i = 0;
...
private Bool ephyrXVPrivGetImageBufSize(int a_port_id, int a_image_id, ushort a_width, ushort a_height, int* a_size)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_xv_query_image_attributes_cookie_t cookie = void;
...
private Bool ephyrXVPrivSaveImageToPortPriv(EphyrPortPriv* a_port_priv, const(ubyte)* a_image_buf, int a_image_len)
{
    Bool is_ok = FALSE;
    ...
    if (a_port_priv.image_buf_size < a_image_len) {
        ...
        buf = cast(ubyte*) realloc(a_port_priv.image_buf, a_image_len);
        if (!buf) {
...
private int ephyrSetPortAttribute(KdScreenInfo* a_info, Atom a_attr_name, int a_attr_value, void* a_port_priv)
{
    ...
    EPHYR_LOG("enter, portnum:%d, atomid:%d, attr_name:%s, attr_val:%d\n",
              ...
              cast(int) a_attr_name, NameForAtom(a_attr_name), a_attr_value);
...
private int ephyrGetPortAttribute(KdScreenInfo* a_screen_info, Atom a_attr_name, int* a_attr_value, void* a_port_priv)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    int res = Success, host_atom = 0;
    EphyrPortPriv* port_priv = a_port_priv;
    ...
    EPHYR_LOG("enter, portnum:%d, atomid:%d, attr_name:%s\n",
              port_priv.port_number,
              cast(int) a_attr_name, NameForAtom(a_attr_name));
...
private void ephyrQueryBestSize(KdScreenInfo* a_info, Bool a_motion, short a_src_w, short a_src_h, short a_drw_w, short a_drw_h, uint* a_prefered_w, uint* a_prefered_h, void* a_port_priv)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    EphyrPortPriv* port_priv = a_port_priv;
    xcb_xv_query_best_size_cookie_t cookie = xcb_xv_query_best_size(conn,
                               port_priv.port_number,
                               a_src_w, a_src_h,
                               a_drw_w, a_drw_h,
                               a_motion);
    xcb_xv_query_best_size_reply_t* reply = xcb_xv_query_best_size_reply(conn, cookie, null);
...
private Bool ephyrHostXVPutImage(KdScreenInfo* a_info, EphyrPortPriv* port_priv, int a_image_id, int a_drw_x, int a_drw_y, int a_drw_w, int a_drw_h, int a_src_x, int a_src_y, int a_src_w, int a_src_h, int a_image_width, int a_image_height, ubyte* a_buf, BoxPtr a_clip_rects, int a_clip_rect_nums)
{
    EphyrScrPriv* scrpriv = a_info.driver;
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_gcontext_t gc = void;
    Bool is_ok = TRUE;
    int data_len = void, width = void, height = void;
    ...
    if (a_clip_rect_nums) {
        xcb_rectangle_t* rects = cast(xcb_rectangle_t*) calloc(a_clip_rect_nums, xcb_rectangle_t.sizeof);
        if (!rects)
            return FALSE;
...
private int ephyrPutImage(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_src_x, short a_src_y, short a_drw_x, short a_drw_y, short a_src_w, short a_src_h, short a_drw_w, short a_drw_h, int a_id, ubyte* a_buf, short a_width, short a_height, Bool a_sync, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrPortPriv* port_priv = a_port_priv;
    Bool is_ok = FALSE;
    ...
    if (!ephyrXVPrivGetImageBufSize(port_priv.port_number,
        ...
        /*this is a minor error so we won't get bail out_ abruptly */
        is_ok = FALSE;
    }
    else {
        is_ok = TRUE;
...
private int ephyrReputImage(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_drw_x, short a_drw_y, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrPortPriv* port_priv = a_port_priv;
...
private int ephyrPutVideo(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_vid_x, short a_vid_y, short a_drw_x, short a_drw_y, short a_vid_w, short a_vid_h, short a_drw_w, short a_drw_h, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrScrPriv* scrpriv = a_info.driver;
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_gcontext_t gc = void;
...
private int ephyrGetVideo(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_vid_x, short a_vid_y, short a_drw_x, short a_drw_y, short a_vid_w, short a_vid_h, short a_drw_w, short a_drw_h, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrScrPriv* scrpriv = a_info.driver;
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_gcontext_t gc = void;
...
private int ephyrPutStill(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_vid_x, short a_vid_y, short a_drw_x, short a_drw_y, short a_vid_w, short a_vid_h, short a_drw_w, short a_drw_h, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrScrPriv* scrpriv = a_info.driver;
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_gcontext_t gc = void;
...
private int ephyrGetStill(KdScreenInfo* a_info, DrawablePtr a_drawable, short a_vid_x, short a_vid_y, short a_drw_x, short a_drw_y, short a_vid_w, short a_vid_h, short a_drw_w, short a_drw_h, RegionPtr a_clipping_region, void* a_port_priv)
{
    EphyrScrPriv* scrpriv = a_info.driver;
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_gcontext_t gc = void;
...
private int ephyrQueryImageAttributes(KdScreenInfo* a_info, int a_id, ushort* a_w, ushort* a_h, int* a_pitches, int* a_offsets)
{
    xcb_connection_t* conn = hostx_get_xcbconn();
    xcb_xv_query_image_attributes_cookie_t cookie = void;
    xcb_xv_query_image_attributes_reply_t* reply = void;
