...
private void exaFillSpans(DrawablePtr pDrawable, GCPtr pGC, int n, DDXPointPtr ppt, int* pwidth, int fSorted)
{
    ...
    int fullX1 = void, fullX2 = void, fullY1 = void;
    int partX1 = void, partX2 = void;
...
private void exaPutImage(DrawablePtr pDrawable, GCPtr pGC, int depth, int x, int y, int w, int h, int leftPad, int format, char* bits)
{
...
Bool exaHWCopyNtoN(DrawablePtr pSrcDrawable, DrawablePtr pDstDrawable, GCPtr pGC, BoxPtr pbox, int nbox, int dx, int dy, Bool reverse, Bool upsidedown)
{
    ...
    xRectangle* rects = cast(xRectangle*) calloc(nbox, xRectangle.sizeof);
    if (rects) {
        int i = void;
        int ordering = void;
    ...
    if (pExaScr.do_migration) {
        ExaMigrationRec[2] pixmaps = void;
        ...
        exaDoMigration(pixmaps.ptr, 2, TRUE);
    }
...
private void exaPolyPoint(DrawablePtr pDrawable, GCPtr pGC, int mode, int npt, DDXPointPtr ppt)
{
    ...
    xRectangle* prect = cast(xRectangle*) calloc(npt, xRectangle.sizeof);
    if (!prect)
        return;
    for (i = 0; i < npt; i++) {
        prect[i].x = ppt[i].x;
...
private void exaPolylines(DrawablePtr pDrawable, GCPtr pGC, int mode, int npt, DDXPointPtr ppt)
{
    ...
    xRectangle* prect = cast(xRectangle*) calloc(npt - 1, xRectangle.sizeof);
    if (!prect)
        return;
    x1 = ppt[0].x;
    y1 = ppt[0].y;
...
{}
    ...
    xRectangle* prect = cast(xRectangle*) calloc(1, cast(uint)nseg * xRectangle.sizeof);
    if (!prect)
        return;
    for (i = 0; i < nseg; i++) {
...
private void exaPolyFillRect(DrawablePtr pDrawable, GCPtr pGC, int nrect, xRectangle* prect)
{
    ...
    BoxPtr pextent = void;
    int extentX1 = void, extentX2 = void, extentY1 = void, extentY2 = void;
    int fullX1 = void, fullX2 = void, fullY1 = void, fullY2 = void;
 ...
 fallback:
    ...
    while (nrect--) {
        fullX1 = prect.x + xorg;
        fullY1 = prect.y + yorg;
        fullX2 = fullX1 + cast(int) prect.width;
        fullY2 = fullY1 + cast(int) prect.height;
        prect++;
