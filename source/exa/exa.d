 ...
 */
c_ulong exaGetPixmapPitch(PixmapPtr pPix)
{
    return pPix.devKind;
}
...
PixmapPtr exaGetDrawablePixmap(DrawablePtr pDrawable)
{
    if (pDrawable.type == DRAWABLE_WINDOW)
        return pDrawable.pScreen.GetWindowPixmap(cast(WindowPtr) pDrawable);
    else
        return cast(PixmapPtr) pDrawable;
}
 ...
 */
Bool exaPixmapHasGpuCopy(PixmapPtr pPixmap)
{
 ...
 */
Bool exaDrawableIsOffscreen(DrawablePtr pDrawable)
{
 ...
 */
private int exaCreateGC(GCPtr pGC)
{
...
private Bool exaChangeWindowAttributes(WindowPtr pWin, c_ulong mask)
{
    Bool ret = void;
...
private RegionPtr exaBitmapToRegion(PixmapPtr pPix)
{
    RegionPtr ret = void;
...
private void ExaWakeupHandler(ScreenPtr pScreen, int result)
{
    ...
    if (result == 0 && pExaScr.numOffscreenAvailable > 1) {
        ...
        if (cast(int) (now - pExaScr.nextDefragment) > 0) {
...
Bool exaDriverInit(ScreenPtr pScreen, ExaDriverPtr pScreenInfo)
{
    ...
    if (!dixRegisterScreenSpecificPrivateKey
        (pScreen, &pExaScr.gcPrivateKeyRec, PRIVATE_GC, ExaGCPrivRec.sizeof)) {
